...
function overview(state) {
  ...
  const standingsRows = standings.map((row, index) => `<tr><td>${index + 1}</td><td>${escapeHtml(row.name)}</td><td>${row.played}</td><td>${row.won}</td><td>${row.drawn}</td><td>${row.lost}</td><td>${row.gf}</td><td>${row.ga}</td><td>${row.gd > 0 ? "+" : ""}${row.gd}</td><td>${row.gf}</td><td><strong>${row.points}</strong></td></tr>`).join("") || `<tr><td colspan="11" class="empty-state">Create teams to see standings.</td></tr>`;
